...
// Sortie attendue: Pierre Dupont: 35000.0€, 1.0 parts
  ...
  println(f"${marie.nom}:")
  println(f"  Quotient familial: ${CalculateurImpot.calculerQuotientFamilial(marie)}%.2f€")
  println(f"  Taux effectif: ${CalculateurImpot.calculerTauxEffectif(marie, 6000)}%.2f%%")
  // Sortie attendue:
  // Marie Martin:
...
def testCalculProgressif(): Unit =
  ...
  testeurs.foreach { foyer =>
    val impot = CalculateurImpot.calculerImpotProgressif(foyer)
    val taux = CalculateurImpot.calculerTauxEffectif(foyer, impot)
...
def testFoyerEnfants(): Unit =
  ...
  foyers.foreach { foyer =>
    val impot = CalculateurImpot.calculerImpotProgressif(foyer)
    val quotient = CalculateurImpot.calculerQuotientFamilial(foyer)
    val taux = CalculateurImpot.calculerTauxEffectif(foyer, impot)
